...
export class ConfigurationManager {
  ...
  //
  // If your _callback requires context it should be bound to that context via .bind()
  public static RequestConfigState(
    _callback: (detail: ConfigState) => void
  ): void {
    if (_callback === null) {
      console.error(
        "Config state request failed. This call requires a callback."
      );
      return;
    }
  ...
  private static BaseConfigChangeRequest(
    ...
    ConnectionManager.serviceConnection()?.SendMessage(
      jsonContent,
      requestID,
      _callback
    );
  ...
  //
  // If your _callback requires context it should be bound to that context via .bind()
  public static RequestConfigFileState(
    _callback: (detail: ConfigState) => void
  ): void {
    if (_callback === null) {
      console.error(
        "Config file state request failed. This call requires a callback."
      );
      return;
    }
