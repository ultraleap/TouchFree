...
export class MessageReceiver {
  ...
  CheckForResponse(): void {
    ...
    if (response !== undefined) {
      const handledResponse = MessageReceiver.HandleCallbackList(
        response,
  ...
  CheckForConfigState(): void {
    ...
    if (configState !== undefined) {
      MessageReceiver.HandleCallbackList(
        configState,
  ...
  CheckForTrackingStateResponse(): void {
    const trackingStateResponse: TrackingStateResponse | undefined =
      this.trackingStateQueue.shift();
  ...
  HandleTrackingStateResponse(
    trackingStateResponse: TrackingStateResponse
  ): void {
  ...
  ClearUnresponsivePromises(): void {
    ...
    MessageReceiver.ClearUnresponsiveItems(
      lastClearTime,
      this.responseCallbacks
    );
    MessageReceiver.ClearUnresponsiveItems(
