...
public class Startup
{
    ...
    private const string RedirectQueryName = "redirect-count";
    ...
    public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
    {
        ...
        app.UseEndpoints(endpoint =>
        {
            ...
            {
                int redirectCount = 0;
                if (context.Request.Query.TryGetValue(RedirectQueryName, out var redirectCountQ))
                {
                    int.TryParse(redirectCountQ.ToString(), out redirectCount);
                }
                ...
                {
                    context.Response.Redirect($"/settings/index.html?{RedirectQueryName}={redirectCount + 1}");
                }
            });
        });
