...
namespace Ultraleap.TouchFree.Tooling
{
    ...
    public class ServiceConnection : IDisposable
    {
        ...
        private void ConnectionResultCallback(ResponseToClient response)
        {
        ...
        public void OnMessage(MessageEventArgs _message)
        {
            ...
            switch (action)
            {
                case ActionCode.INPUT_ACTION:
                    WebsocketInputAction wsInput = JsonConvert.DeserializeObject<WebsocketInputAction>(content);
                ...
                case ActionCode.CONFIGURATION_STATE:
                    ConfigState configState = JsonConvert.DeserializeObject<ConfigState>(content);
                    ConnectionManager.messageReceiver.configStateQueue.Enqueue(configState);
                    break;
                case ActionCode.HAND_PRESENCE_EVENT:
                    HandPresenceEvent handEvent = JsonConvert.DeserializeObject<HandPresenceEvent>(content);
                ...
                case ActionCode.VERSION_HANDSHAKE_RESPONSE:
                    ResponseToClient response = JsonConvert.DeserializeObject<ResponseToClient>(content);
        ...
        // be given a pre-made _message and _requestID. Provides an asynchronous <ResponseToClient>
        // via the _callback parameter.
        internal void SendMessage(string _message, string _requestID, Action<ResponseToClient> _callback)
        {
            if (_requestID == "")
            {
                if (_callback != null)
                {
                    ResponseToClient response = new ResponseToClient("", "Failure", "Request failed. This is due to a missing or invalid requestID", _message);
                    _callback.Invoke(response);
                }

                Debug.WriteLine("Request failed. This is due to a missing or invalid requestID");
                return;
            }
