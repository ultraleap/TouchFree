...
export class ServiceConnection {
  ...
  constructor(_ip: string = "127.0.0.1", _port: string = "9739") {
    ...
    this.webSocket.addEventListener("open", this.RequestHandshake.bind(this), {
      once: true,
    });
  }
  ...
  private RequestHandshake() {
    if (!this.handshakeCompleted) {
      ...
      handshakeRequest.content[VersionInfo.API_HEADER_NAME] =
  ...
  private ConnectionResultCallback(response: WebSocketResponse): void {
    if (response.status === "Success") {
      ...
      console.log(
        `Connection to Service failed. Details:\n${response.message}`
      );
    }
  }
  ...
  OnMessage(_message: MessageEvent): void {
    ...
    switch (looseData.action as ActionCode) {
      ...
      case ActionCode.SERVICE_STATUS:
        ...
        ConnectionManager.messageReceiver.serviceStatusQueue.push(
          serviceStatus
        );
        break;
      ...
      case ActionCode.CLOSE_TO_SWIPE:
        ...
        InputActionManager.HandleCloseToSwipe(looseData.content);
        break;
      ...
      case ActionCode.QUICK_SETUP_CONFIG:
        ...
        ConnectionManager.messageReceiver.configStateQueue.push(
          configFileState
        );
        break;
  ...
  RequestConfigState(_callback: (detail: ConfigState) => void): void {
    if (_callback === null) {
      console.error(
        "Request for config state failed. This is due to a missing callback"
      );
      return;
    }
    ...
    let wrapper: CommunicationWrapper<any> =
      new CommunicationWrapper<ConfigChangeRequest>(
  ...
  RequestServiceStatus(_callback: (detail: ServiceStatus) => void): void {
    if (_callback === null) {
      console.error(
        "Request for service status failed. This is due to a missing callback"
      );
      return;
    }
  ...
  RequestConfigFile(_callback: (detail: ConfigState) => void): void {
    if (_callback === null) {
      console.error(
        "Request for config file failed. This is due to a missing callback"
      );
      return;
    }
  ...
  QuickSetupRequest(
    ...
    let wrapper: CommunicationWrapper<any> = new CommunicationWrapper<any>(
  ...
  RequestTrackingState(_callback: (detail: TrackingStateResponse) => void) {
    if (!_callback) {
      console.error(
        "Request for tracking state failed. This is due to a missing callback"
      );
      return;
    }
    const guid: string = uuidgen();
    const request: SimpleRequest = new SimpleRequest(guid);
    const wrapper: CommunicationWrapper<any> =
      new CommunicationWrapper<SimpleRequest>(
  ...
  RequestTrackingChange(
    ...
    const wrapper: CommunicationWrapper<Partial<TrackingStateRequest>> =
      new CommunicationWrapper<Partial<TrackingStateRequest>>(
