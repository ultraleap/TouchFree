...
public class CallToInteractController : MonoBehaviour
{
    ...
    void UpdateCTI()
    {
        ...
        {
            var handIsPresent = Leap.Unity.Hands.Provider.CurrentFrame.Hands.Count > 0;
            if (handIsPresent)
            {
                showTimer.Reset();

                if (!handWasPresentLastFrame)
                {
                    OnHandEnter();
                }
            }
            else if (!handIsPresent && handWasPresentLastFrame)
            {
                OnAllHandsExit();
            }

            handWasPresentLastFrame = handIsPresent;

            if (!IsShowing && showTimer.Elapsed.TotalSeconds > ShowTimeAfterNoHandPresent && !ConfigurationSetupController.isActive)
            {
                showTimer.Reset();
                ShowCTI();
            }
